...
train_rf_model <- function(training_set, validation_set, testing_set) {
  ...
  rf_model <- train(tumor_type ~ . - tumor_label - row_id,
                    data = training_set,
                    method = "ranger", 
  ...
  y_pred_train_prob <- predict(rf_model, training_set, type = "prob")
  y_pred_train_class <- predict(rf_model, training_set, type = "raw")
  
  rf_cm_train <- confusionMatrix(y_pred_train_class, training_set$tumor_type)
  ...
  plot_multiclass_roc(y_pred_train_prob, training_set$tumor_type, "Random Forest Training Set")
  ...
  rf_tune_model <- train(tumor_type ~ . - tumor_label - row_id,
                         data = training_set,
                         method = "ranger", 
  ...
  y_pred_val_prob <- predict(rf_tune_model, validation_set, type = "prob")
  y_pred_val_class <- predict(rf_tune_model, validation_set, type = "raw")
  
  rf_cm_val <- confusionMatrix(y_pred_val_class, validation_set$tumor_type)
  ...
  plot_multiclass_roc(y_pred_val_prob, validation_set$tumor_type, "Random Forest Validation Set")
  
  # ----------------- Testing the Tuned Model (Test Set) ----------------- #
  
  y_pred_test_prob <- predict(rf_tune_model, testing_set, type = "prob")
  y_pred_test_class <- predict(rf_tune_model, testing_set, type = "raw")
  
  rf_cm_test <- confusionMatrix(y_pred_test_class, testing_set$tumor_type)
  ...
  plot_multiclass_roc(y_pred_test_prob, testing_set$tumor_type, "Random Forest Test Set")
  ...
  plot_ovr_roc <- function(prob_matrix, true_labels, dataset_name, file_name) {
    ...
    plot(0, 0,
         ...
         main = paste("One-vs-All ROC Curves for Random Forest", dataset_name),
         cex.main = 1.1 * scale,
         cex.lab = 1.1 * scale,
         cex.axis = 1.1 * scale)
  ...
  plot_ovr_roc(y_pred_train_prob,
               ...
               "Training Set",
               "D:/CPE407/THESIS/Result Multi/rf_roc_multi_train.png")
  
  # Validation
  plot_ovr_roc(y_pred_val_prob,
               validation_set$tumor_type,
               "Validation Set",
               "D:/CPE407/THESIS/Result Multi/rf_roc_multi_validation.png")
  
  # Test
  plot_ovr_roc(y_pred_test_prob,
               testing_set$tumor_type,
               "Test Set",
               "D:/CPE407/THESIS/Result Multi/rf_roc_multi_test.png")
  
  # ----------------- Prepare a Results DataFrame ----------------- #
  
  per_class_metrics_train <- calculate_metrics_by_class(rf_cm_train)
  per_class_metrics_val <- calculate_metrics_by_class(rf_cm_val)
  per_class_metrics_test <- calculate_metrics_by_class(rf_cm_test)
  per_class_all <- rbind(
  ...
  overall_summary_df <- data.frame(
    ...
    Accuracy = c(rf_cm_train$overall["Accuracy"],
                 rf_cm_val$overall["Accuracy"],
                 rf_cm_test$overall["Accuracy"]),
    CI_Lower = c(rf_cm_train$overall["AccuracyLower"],
                 rf_cm_val$overall["AccuracyLower"],
                 rf_cm_test$overall["AccuracyLower"]),
    CI_Upper = c(rf_cm_train$overall["AccuracyUpper"],
                 rf_cm_val$overall["AccuracyUpper"],
                 rf_cm_test$overall["AccuracyUpper"]),
