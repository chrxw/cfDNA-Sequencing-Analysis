...
train_knn_model <- function(training_set, validation_set, testing_set) {
  ...
  knn_model <- train(tumor_type ~ . - tumor_label - row_id,
                     data = training_set,
                     method = "knn", 
  ...
  y_pred_train_prob <- predict(knn_model, training_set, type = "prob") 
  y_pred_train_class <- predict(knn_model, training_set, type = "raw")
  
  knn_cm_train <- confusionMatrix(y_pred_train_class, training_set$tumor_type)
  ...
  plot_multiclass_roc(y_pred_train_prob, training_set$tumor_type, "KNN Training Set")
  
  # ----------------- Hyperparameter Tuning (Validation Set) ----------------- #
  
  tune_grid_tune_knn <- data.frame(k = seq(3, 101, by = 2))
  
  knn_tune_model <- train(tumor_type ~ . - tumor_label - row_id,
                          data = training_set,
                          method = "knn", 
  ...
  y_pred_val_prob <- predict(knn_tune_model, validation_set, type = "prob")
  y_pred_val_class <- predict(knn_tune_model, validation_set, type = "raw")
  
  knn_cm_val <- confusionMatrix(y_pred_val_class, validation_set$tumor_type)
  ...
  plot_multiclass_roc(y_pred_val_prob, validation_set$tumor_type, "KNN Validation Set")
  
  # ----------------- Testing the Tuned Model (Test Set) ----------------- #
  
  y_pred_test_prob <- predict(knn_tune_model, testing_set, type = "prob")
  y_pred_test_class <- predict(knn_tune_model, testing_set, type = "raw")
  
  knn_cm_test <- confusionMatrix(y_pred_test_class, testing_set$tumor_type)
  ...
  plot_multiclass_roc(y_pred_test_prob, testing_set$tumor_type, "KNN Test Set")
  ...
  plot_ovr_roc <- function(prob_matrix, true_labels, dataset_name, file_name) {
    ...
    plot(0, 0,
         ...
         main = paste("One-vs-All ROC Curves for KNN", dataset_name),
         cex.main = 1.1 * scale,
         cex.lab = 1.1 * scale,
         cex.axis = 1.1 * scale)
  ...
  plot_ovr_roc(y_pred_train_prob,
               ...
               "Training Set",
               "D:/CPE407/THESIS/Result Multi/knn_roc_multi_train.png")
  
  # Validation
  plot_ovr_roc(y_pred_val_prob,
               validation_set$tumor_type,
               "Validation Set",
               "D:/CPE407/THESIS/Result Multi/knn_roc_multi_validation.png")
  
  # Test
  plot_ovr_roc(y_pred_test_prob,
               testing_set$tumor_type,
               "Test Set",
               "D:/CPE407/THESIS/Result Multi/knn_roc_multi_test.png")
  
  # ----------------- Prepare a Results DataFrame ----------------- #
  
  per_class_metrics_train <- calculate_metrics_by_class(knn_cm_train)
  per_class_metrics_val <- calculate_metrics_by_class(knn_cm_val)
  per_class_metrics_test <- calculate_metrics_by_class(knn_cm_test)
  per_class_all <- rbind(
  ...
  overall_summary_df <- data.frame(
    ...
    Accuracy = c(knn_cm_train$overall["Accuracy"],
                 knn_cm_val$overall["Accuracy"],
                 knn_cm_test$overall["Accuracy"]),
    CI_Lower = c(knn_cm_train$overall["AccuracyLower"],
                 knn_cm_val$overall["AccuracyLower"],
                 knn_cm_test$overall["AccuracyLower"]),
    CI_Upper = c(knn_cm_train$overall["AccuracyUpper"],
                 knn_cm_val$overall["AccuracyUpper"],
                 knn_cm_test$overall["AccuracyUpper"]),
