...
train_knn_model <- function(training_set, validation_set, testing_set, name) {
  ...
  y_pred_train_class <- predict(knn_model, training_set, type = "raw")          # Hard predictions
  ...
  knn_sensitivity <- knn_cm_train$byClass["Sensitivity"]
  knn_specificity <- knn_cm_train$byClass["Specificity"]
  knn_f1_score <- knn_cm_train$byClass["F1"]
  
  # Plot ROC curve for training set
  plot.roc(roc_train, col = "blue", main = paste("ROC Curve for the KNN Model (Training Set):", name))
  ...
  auc_val <- roc_val$auc
  knn_tune_sensitivity <- knn_cm_val$byClass["Sensitivity"]
  knn_tune_specificity <- knn_cm_val$byClass["Specificity"]
  knn_tune_f1_score <- knn_cm_val$byClass["F1"]
  
  # Plot ROC curve for validation set
  plot.roc(roc_val, col = "green", main = paste("ROC Curve for the KNN Model (Validation Set):", name))
  ...
  knn_test_sensitivity <- knn_cm_test$byClass["Sensitivity"]
  knn_test_specificity <- knn_cm_test$byClass["Specificity"]
  knn_test_f1_score <- knn_cm_test$byClass["F1"]
  
  # Plot ROC curve for test set
  plot.roc(roc_test, col = "red", main = paste("ROC Curve for the KNN Model (Test Set):", name))
  ...
  plot_combined_roc <- function(name, file_name) {
    ...
    plot(0, 0,
         ...
         main = paste("ROC Curves for KNN Model:", name),
         cex.main = 1.1 * scale,
         cex.lab = 1.1 * scale,
         cex.axis = 1.1 * scale)
  ...
  plot_combined_roc(
    name,
    paste0("D:/CPE407/THESIS/Result Bi/knn_roc_bi_", name, ".png")
  )
