...
processed_dataframe <- processed_dataframe %>%
  mutate(
    ...
    tumor_label = ifelse(tumor_type == "Healthy", "no_tumor", "tumor"),
    row_id = seq_len(n())
  )

processed_dataframe$tumor_type  <- as.factor(processed_dataframe$tumor_type)
processed_dataframe$tumor_label <- as.factor(processed_dataframe$tumor_label)
...
split1 <- split_testing_set(processed_dataframe, test_prop = 0.30)
...
dataset <- prepare_dataset(remaining_original)
...
complete_dataset <- rbind(training_set, validation_set, testing_set)
...
# Save plot
tumor_distribution_plots_ieee(complete_dataset,
                              output_path = "D:/CPE407/THESIS/Result Normal/tumor_distribution_ieee.png")
...
create_stacked_distribution_plot_ieee(training_set, validation_set, testing_set,
                                      output_path = "D:/CPE407/THESIS/Result Normal/stacked_distribution_ieee.png")
